...
> ✅ **ALL 8 FILES LOADED + VALIDATED — 2026-06-07.** See `LOAD-LOG.md` for the final footprint + isolation proof.
...
| 01 | `01_entities.sql` | `seo_entity_graph` | 163 authored → **+113 net-new** (50 shared `['*']` reused); global 722→835 | ✅ done |
| 02 | `02_entity_extensions.sql` | condition/symptom/anatomy/procedures/drug | SmileScape-scoped: condition 28 + procedure 46 + anatomy 6 | ✅ done |
| 03 | `03_citations.sql` | `seo_citations` | 93 → **+91** (2 DOI already in pool); `ours_present`=91 | ✅ done |
| 04 | `04_authors.sql` | `seo_authors_reviewers` + `seo_doctor_assignments` | +2 authors +2 assignments | ✅ done |
| 05 | `05_branches.sql` | `seo_branches` | +2 (partial; org-link both) | ✅ done |
| 06 | `06_pages.sql` | `seo_website_page_master` | +722 stub (721 entity-bound; expect `orphan_entity`=3 → orthodontic-intervention) | ✅ done |
| 10 | `10_keywords.sql` | `seo_x_ads_keywords_contextual_master` | +525 seed (brand `Smile Scape Clinic`) → DFS | ✅ done |
